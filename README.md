...
If more than one MPI implementation is installed (e.g. a system OpenMPI package alongside a `conda`-bundled MPI and/or an HPC-SDK one), also pin `MPI_CXX_COMPILER` explicitly rather than letting `find_package(MPI)` pick whichever `mpicxx` happens to be first on `PATH`. Picking the wrong one links a working binary that still crashes at runtime (`version 'GLIBCXX_...' not found`, or two different `libmpi.so` sonames loaded at once) because `libsc`/GTS were built against a specific MPI's ABI:

```
>> cmake .. -DMPI_CXX_COMPILER=/usr/bin/mpicxx.openmpi -DSC_DIR=... -DHDF5_ROOT=...
```
...
* `MPI_CXX_COMPILER` — pin the exact `mpicxx` to use when more than one MPI implementation is installed.
...
HexMesh is driven by a single config file, inspired by the `HexMesh.input` mechanism on the upstream [jcamata/HexMesh](https://github.com/jcamata/HexMesh) `smart_octree` branch. Copy the example [`HexMesh.input`](HexMesh.input) at the repository root, edit it for your run, then:

```
>> mpirun -np <nb_proc> ./hexmesh [config_file]
```

`[config_file]` is optional and defaults to `./HexMesh.input`; `<nb_proc>` is the number of MPI processes used to create the mesh (each process writes its own output file).

The config file is a flat `key = value` list; `#` starts a comment, blank lines are ignored, and unknown keys or malformed lines produce a warning rather than being silently dropped.

| Key | Required | Meaning |
| --- | --- | --- |
| `ref` | yes | Refinement level of the 27-tree structure (an integer corresponding to a power of 3: 4, 5, 6, ...). |
| `topo` | yes | Path to the GTS file for the topography surface. |
| `outmesh` | yes | Output mesh file prefix. |
| `n_pml_layers` | yes | Number of PML element layers extruded on each side of the domain. |
| `pml_length` | yes | Physical thickness (in meters) of the PML region on each side of the domain. |
| `bathy` | no | Path to the GTS file for the bathymetry surface. Leave blank or omit it entirely to generate the mesh without a bathymetry cut: the element interception, node-projection and pillowing steps are then skipped, and the soil mesh (with its PML) is generated directly from the topography surface. |

A missing required key, an unreadable config file, or a value that fails to parse (e.g. `n_pml_layers = three`) is reported with the offending file/line before HexMesh exits, rather than failing deep inside mesh generation.

`topo`/`bathy` are produced from STL files with the `stl2gts` tool (installed alongside GTS); the STL files themselves are generated by the Python API in `easyrisk-post`, which replaces the old `mainSRTM.m` Matlab pipeline.
